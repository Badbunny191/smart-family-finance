  ...
  return (
    <main className="app-shell min-h-screen pb-24">
      <header className="sticky top-0 z-10 border-b border-slate-200/70 bg-white/90 px-5 pb-5 pt-6 backdrop-blur-xl">
        <div className="flex items-center justify-between gap-4"><div><p className="section-label">แหล่งเงินของครอบครัว</p><h1 className="mt-2 text-[1.65rem] font-bold tracking-tight text-slate-900">บัญชี</h1></div><button type="button" onClick={openCreate} disabled={people.length === 0} className="touch-button flex items-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-semibold text-white shadow-[0_8px_18px_rgba(22,134,107,0.2)] disabled:cursor-not-allowed disabled:opacity-50"><Plus size={18} aria-hidden="true" /> เพิ่ม</button></div>
      </header>

      <section className="space-y-3 px-5 py-5">
            {isLoading ? <p className="py-12 text-center text-sm text-slate-500">กำลังโหลด...</p> : accounts.length === 0 ? <div className="surface-card border-dashed px-5 py-12 text-center"><WalletCards className="mx-auto text-slate-400" size={30} aria-hidden="true" /><p className="mt-3 font-medium text-slate-700">ยังไม่มีบัญชี</p><p className="mt-1 text-sm text-slate-500">เพิ่มบุคคลก่อนเพื่อสร้างบัญชีแรก</p></div> : accounts.map((account) => <article key={account.id} className="surface-card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-3"><div className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${account.accountType === 'bank' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-700'}`}>{account.accountType === 'bank' ? <WalletCards size={21} /> : <Banknote size={21} />}</div><div className="min-w-0"><h2 className="truncate font-semibold text-slate-900">{account.name}</h2><p className="mt-1 text-xs text-slate-500">{account.accountType === 'bank' ? 'ธนาคาร' : 'เงินสด'} · {account.personName}</p></div></div><p className="mt-5 text-2xl font-bold tracking-tight text-slate-900">{account.currentBalance.toLocaleString('th-TH', { minimumFractionDigits: 2 })} <span className="text-sm font-medium text-slate-500">บาท</span></p>{account.propertyName && <p className="mt-1 text-xs text-slate-500">ผูกกับ: {account.propertyName}</p>}</div><div className="flex gap-2"><button type="button" onClick={() => openEdit(account)} aria-label={`แก้ไข ${account.name}`} className="touch-button grid min-w-11 place-items-center rounded-xl bg-slate-100 text-slate-600"><Pencil size={18} aria-hidden="true" /></button><button type="button" onClick={() => deleteAccount(account.id)} aria-label={`ลบ ${account.name}`} className="touch-button grid min-w-11 place-items-center rounded-xl bg-rose-50 text-rose-600"><Trash2 size={18} aria-hidden="true" /></button></div></div></article>)}
          </section>
