      ...
      <div className="mx-auto max-w-5xl space-y-8 px-5 py-6">
        <section>
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <a href="/transactions?businessStatus=customer_paid" className="surface-card block overflow-hidden p-4 transition-transform active:scale-[0.98]"><div className="grid min-h-10 min-w-10 w-fit place-items-center rounded-2xl bg-amber-50 text-amber-700"><ArrowLeftRight size={20} /></div><p className="mt-3 text-xs font-medium text-slate-500">รอโอนเข้าธุรกิจ</p><p className="mt-1 truncate text-lg font-bold tracking-tight text-slate-900">{formatCurrency(data.summary.pendingBusinessTransfer)}</p><p className="mt-1 text-xs font-medium text-amber-700">{data.summary.pendingBusinessTransferCount} รายการ</p></a>
        ...
        <section>
          ...
          {data.recentTransactions.length === 0 ? <EmptyState title="ยังไม่มีรายการเงิน" description="เริ่มต้นด้วยการเพิ่มรายรับหรือรายจ่าย" href="/transactions" action="เพิ่มรายการ" /> : <div className="grid gap-3 lg:grid-cols-2">{data.recentTransactions.map((transaction) => <article key={transaction.id} className="surface-card overflow-hidden p-4"><div className="flex items-start gap-3"><div className={`grid min-h-10 min-w-10 place-items-center rounded-2xl ${transaction.type === 'income' ? 'bg-emerald-50 text-emerald-700' : transaction.type === 'expense' ? 'bg-rose-50 text-rose-700' : 'bg-indigo-50 text-indigo-700'}`}>{transaction.type === 'income' ? <ArrowDownLeft size={19} /> : transaction.type === 'expense' ? <ArrowUpRight size={19} /> : <ArrowLeftRight size={19} />}</div><div className="min-w-0 flex-1 overflow-hidden"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-semibold text-slate-900">{transaction.title}</h3><p className="mt-1 text-xs text-slate-500">{typeLabels[transaction.type]} · {new Date(transaction.date).toLocaleDateString('th-TH')}</p></div><p className={`shrink-0 font-bold ${transaction.type === 'expense' ? 'text-rose-700' : transaction.type === 'transfer' ? 'text-indigo-700' : 'text-emerald-700'}`}>{transaction.type === 'expense' ? '-' : '+'}{formatCurrency(transaction.amount)}</p></div><p className="mt-2 truncate text-xs text-slate-500">{transaction.type === 'transfer' ? `${transaction.sourceAccountName || '-'} → ${transaction.destinationAccountName || '-'}` : transaction.sourceAccountName || transaction.destinationAccountName || 'ไม่ระบุบัญชี'}</p></div></div></article>)}</div>}
        </section>
...
function SummaryCard({ label, value, icon, tone }: { label: string; value: number; icon: React.ReactNode; tone: 'emerald' | 'sky' | 'rose' }) {
  ...
  return <article className="surface-card overflow-hidden p-4"><div className={`grid min-h-10 min-w-10 w-fit place-items-center rounded-2xl ${toneClass}`}>{icon}</div><p className="mt-3 text-xs font-medium text-slate-500">{label}</p><p className="mt-1 truncate text-lg font-bold tracking-tight text-slate-900">{formatCurrency(value)}</p></article>;
}
