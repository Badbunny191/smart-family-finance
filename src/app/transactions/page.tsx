    ...
    <main className="app-shell min-h-screen pb-24">
      <header className="sticky top-0 z-10 border-b border-slate-200/70 bg-white/90 px-5 pb-5 pt-6 backdrop-blur-xl">
        <div className="flex items-center justify-between gap-4">
          ...
          <button type="button" onClick={() => openCreate()} disabled={people.length === 0 || accounts.length === 0} className="touch-button grid min-w-11 place-items-center rounded-xl bg-emerald-600 text-white shadow-[0_8px_18px_rgba(22,134,107,0.2)] disabled:cursor-not-allowed disabled:opacity-50" aria-label="เพิ่มรายการ"><Plus size={20} aria-hidden="true" /></button>
        </div>
        <div className="mt-5 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1">
          {typeOptions.map(({ value, label, icon: Icon }) => <button key={value} type="button" onClick={() => openCreate(value)} className={`touch-button flex items-center justify-center gap-1 rounded-xl text-xs font-semibold ${form.type === value && isFormOpen ? value === 'income' ? 'bg-white text-emerald-700 shadow-sm' : value === 'expense' ? 'bg-white text-rose-700 shadow-sm' : 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}><Icon size={16} aria-hidden="true" />{label}</button>)}
        </div>
      </header>

      <section className="space-y-3 px-5 py-5">
        {errorMessage && <p className="rounded-2xl bg-rose-50 p-4 text-sm text-rose-700">{errorMessage}</p>}
        {isLoading ? <p className="py-12 text-center text-sm text-slate-500">กำลังโหลด...</p> : transactions.length === 0 ? <div className="surface-card border-dashed px-5 py-12 text-center"><ArrowLeftRight className="mx-auto text-slate-400" size={30} aria-hidden="true" /><p className="mt-3 font-medium text-slate-700">ยังไม่มีรายการเงิน</p><p className="mt-1 text-sm text-slate-500">เพิ่มรายรับ รายจ่าย หรือรายการโอนได้จากปุ่มด้านบน</p></div> : transactions.map((transaction) => <article key={transaction.id} className="surface-card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`grid min-h-10 min-w-10 place-items-center rounded-2xl ${transaction.type === 'income' ? 'bg-emerald-50 text-emerald-700' : transaction.type === 'expense' ? 'bg-rose-50 text-rose-700' : 'bg-indigo-50 text-indigo-700'}`}>{transaction.type === 'income' ? <ArrowDownLeft size={18} /> : transaction.type === 'expense' ? <ArrowUpRight size={18} /> : <ArrowLeftRight size={18} />}</span><div><h2 className="truncate font-semibold text-slate-900">{transaction.title}</h2><p className="text-xs text-slate-500">{new Date(transaction.date).toLocaleDateString('th-TH')}</p></div></div><p className={`mt-3 text-xl font-bold tracking-tight ${transaction.type === 'expense' ? 'text-rose-700' : transaction.type === 'transfer' ? 'text-indigo-700' : 'text-emerald-700'}`}>{transaction.type === 'expense' ? '-' : '+'}{transaction.amount.toLocaleString('th-TH', { minimumFractionDigits: 2 })} บาท</p><p className="mt-1 text-xs text-slate-500">{personName.get(transaction.ownerPersonId) || 'ไม่ระบุ'} · {transaction.type === 'transfer' ? `${accountName.get(transaction.sourceAccountId || '') || '-'} → ${accountName.get(transaction.destinationAccountId || '') || '-'}` : accountName.get(transaction.sourceAccountId || transaction.destinationAccountId || '') || 'ไม่ระบุบัญชี'}</p>{transaction.propertyId && <p className="mt-1 text-xs text-slate-500">ทรัพย์สิน: {propertyName.get(transaction.propertyId) || '-'}</p>}</div><button type="button" onClick={() => deleteTransaction(transaction.id)} aria-label={`ลบ ${transaction.title}`} className="touch-button grid min-w-11 place-items-center rounded-xl bg-rose-50 text-rose-600"><Trash2 size={18} aria-hidden="true" /></button></div></article>)}
      </section>
