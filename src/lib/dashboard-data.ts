...
export type DashboardData = {
  summary: { totalAssets: number; totalCash: number; monthlyIncome: number; monthlyExpense: number; netBalance: number; pendingBusinessTransfer: number; pendingBusinessTransferCount: number };
...
export async function getDashboardData(db: AppDatabase): Promise<DashboardData> {
  ...
  const [balanceRow, incomeRow, expenseRow, pendingTransferRow, pendingTransferCountRow, recentRows, accountRows, propertyRows] = await Promise.all([
    ...
    db.select({ total: sql<number>`count(*)` }).from(transactions).where(and(eq(transactions.type, 'income'), eq(transactions.businessStatus, 'customer_paid'), eq(transactions.status, 'completed'), isNull(transactions.deletedAt))),
  ...
  const pendingBusinessTransferCount = Number(pendingTransferCountRow[0]?.total || 0);

  return {
    summary: { totalAssets, totalCash, monthlyIncome, monthlyExpense, netBalance: monthlyIncome - monthlyExpense, pendingBusinessTransfer, pendingBusinessTransferCount },
