...
export type DashboardData = {
  summary: { totalAssets: number; totalCash: number; monthlyIncome: number; monthlyExpense: number; netBalance: number };
...
export async function getDashboardData(db: AppDatabase): Promise<DashboardData> {
  ...
  const [balanceRow, incomeRow, expenseRow, recentRows, accountRows, propertyRows] = await Promise.all([
  ...
  return {
    summary: { totalAssets, totalCash, monthlyIncome, monthlyExpense, netBalance: monthlyIncome - monthlyExpense },
